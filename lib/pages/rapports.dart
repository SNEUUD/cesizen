...
Future<List<Rapport>> fetchRapports(String userId) async {
  final response = await http.get(
    Uri.parse('https://127.0.0.1:3050/rapports_user?userId=$userId'),
  );
...
Future<List<Emotion>> fetchEmotions() async {
  final response = await http.get(Uri.parse('https://127.0.0.1:3050/emotions'));
  if (response.statusCode == 200) {
    final List data = jsonDecode(response.body);
...
Future<void> deleteRapport(int id, BuildContext context) async {
  final response = await http.delete(
    Uri.parse('https://127.0.0.1:3050/delete_rapports/$id'),
  );
  if (response.statusCode != 200) {
